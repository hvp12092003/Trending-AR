...
public class MainMenuDataManager : MonoBehaviour
{
    ...
    // Helper variables to detect tap/drag for tap audio
    private const float TAP_DRAG_THRESHOLD = 15f;
    ...
    private void Update()
    {
        ...
        if (!playedTouchAudio && Mouse.current != null)
        {
            if (Mouse.current.leftButton.wasPressedThisFrame)
            {
                m_MouseStartPos = Mouse.current.position.ReadValue();
            ...
            if (m_IsMouseActive)
            {
                ...
                if (Vector2.Distance(currentPos, m_MouseStartPos) > TAP_DRAG_THRESHOLD)
                {
...
public class EditorBandData
{
    ...
    [Tooltip("Tên State hoạt ảnh nhảy chung của tất cả thành viên trong ban nhạc (ví dụ: Dance1, Dance2, Dance3)")]
    public string danceAnimId;
