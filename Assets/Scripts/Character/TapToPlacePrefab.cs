...
public class TapToPlacePrefab : MonoBehaviour
{
    ...
    private void Update()
    {
                    ...
                    if (characterMove != null)
                    {
                        ...
                        if (spawner != null)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // Chưa kéo ngay lập tức, lưu lại vị trí chạm để kiểm tra threshold
        ...
        if (IsPointerReleasedThisFrame())
        {
            ...
            {
                // Nhấn và thả mà không kéo quá threshold -> Đây là hành động Click/Tap chọn nhân vật
                if (CharacterManager.Instance != null)
                {
    ...
    private void UpdateDragPosition(Vector2 pointerPos)
    {
        ...
        Vector3 targetScale = m_OriginalDraggedScale != Vector3.zero ? m_OriginalDraggedScale : GetPlacedScale();
