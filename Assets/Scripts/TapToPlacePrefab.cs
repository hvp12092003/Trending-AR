...
public class TapToPlacePrefab : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (GetInputPose(out Vector2 touchPosition))
        {
            ...
            // 2. Thực hiện Raycast từ điểm chạm vào thế giới AR nếu không nhấp trúng nhân vật nào
            if (m_UseFallbackMode)
            {
                if (m_MainCamera != null)
                {
                    Ray ray = m_MainCamera.ScreenPointToRay(touchPosition);
                    // Giả lập mặt đất cách camera ảo 1.2 mét
                    Plane virtualPlane = new Plane(Vector3.up, new Vector3(0, -1.2f, 0));
                    if (virtualPlane.Raycast(ray, out float enter))
                    {
                        Vector3 hitPoint = ray.GetPoint(enter);
                        Pose hitPose = new Pose(hitPoint, Quaternion.identity);
    ...
    private void HandleSpawning(Pose hitPose)
    {
        ...
        {
            // Chế độ tạo nhiều: Mỗi lần chạm tạo ra một bản sao mới
        ...
        else if (m_SpawnMode == SpawnMode.MoveSingle)
        {
            ...
            else
            {
                ...
                m_SpawnedObject.transform.rotation = spawnRotation;
